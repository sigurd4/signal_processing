...
use crate::{validate_filter_bands, generators::filter::{FilterGenPlane, FilterGenType, FilterBandError}};

pub fn cheb2ord<T, const F: usize>(
    ...
    [(); 2 - F]:
{
    ...
    match filter_type
    {
        FilterGenType::LowPass => {
            wcw_p = passband_frequencies.map(|w| w*k);
            wcw_s = stopband_frequencies;
        },
        ...
        FilterGenType::BandPass | FilterGenType::BandStop => {
            ...
            {
                w_prime_p = passband_frequencies.map(|w| w*k);
                w_prime_s = stopband_frequencies;
            }
            else
            {
                w_prime_p = passband_frequencies.map(|w| w/k);
    ...
    if let Some(t) = t
    {
        Ok((n, wcw_p.map(|w| (w*t/two).atan()*two/T::PI()), wcw_s.map(|w| (w*t/two).atan()*two/T::PI()), stopband_attenuation, filter_type))
    }
    else
    {
        Ok((n, wcw_p, wcw_s, stopband_attenuation, filter_type))
    }
}