...
pub fn fst_iv_unscaled<B, C, T>(sequence: &mut B, mut temp: Option<&mut [C]>)
where
...
{
    if dst_iv_fft_unscaled(sequence, &mut temp)
    {
        return
    }
    dst_iv_direct_unscaled(sequence, &mut temp);
}

pub fn dst_iv_fft_unscaled<B, C, T>(sequence: &mut B, temp: &mut Option<&mut [C]>) -> bool
where
    for<'a> &'a mut B: IntoBulk<Item: BorrowMut<C>>,
    B: ?Sized,
    C: ComplexFloat<Real = T>,
    T: Float + FloatConst + 'static
{
    ...
    let shift = -Complex::cis(T::FRAC_PI_4()/lenf)*Complex::i();
    temp.bulk_mut()
...
pub fn dst_iv_direct_unscaled<B, C, T>(sequence: &mut B, temp: &mut Option<&mut [C]>)
where
    for<'a> &'a mut B: IntoBulk<Item: BorrowMut<C>>,
    B: ?Sized,
    C: ComplexFloat<Real = T>,
