...
) -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let x_min = x.iter().copied().reduce(T::min).unwrap();
    let x_max = x.iter().copied().reduce(T::max).unwrap();
    
    let y_min = y.iter().copied().reduce(T::min).unwrap();
    let y_max = y.iter().copied().reduce(T::max).unwrap();
...
) -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let ([x_min, y_min, z_min], [x_max, y_max, z_max]) = f_values.into_iter()
        .map(|f| (f, f))
        .reduce(|a, b| (a.0.into_bulk().zip(b.0).map(|(a, b)| a.min(b)).collect(), a.1.into_bulk().zip(b.1).map(|(a, b)| a.max(b)).collect()))
        .unwrap();
