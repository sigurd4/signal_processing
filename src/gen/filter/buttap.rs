...
mod test
{
    ...
    use crate::{plot, Bilinear, ButtAP, FreqS, Plane, RealFreqZ, Zpk};
    ...
    fn test()
    {
        ...
        plot::plot_pz("H(s)", "plots/pz_s_buttap.png", h.poles(), h.zeros(), Plane::S)
            .unwrap();
        ...
        plot::plot_curves("H(jw)", "plots/h_s_buttap.png", [&w.zip(h_f.map(|h| h.norm())), &w.zip(h_f.map(|h| h.arg()))])
            .unwrap();
        
        let h = h.bilinear(fs)
            .unwrap();

        plot::plot_pz("H(z)", "plots/pz_z_buttap.png", h.poles(), h.zeros(), Plane::Z)
            .unwrap();
        
        let (h_f, w): ([_; N], _) = h.real_freqz(());

        plot::plot_curves("H(e^jw)", "plots/h_z_buttap.png", [&w.zip(h_f.map(|h| h.norm())), &w.zip(h_f.map(|h| h.arg()))])
            .unwrap();
    }
}