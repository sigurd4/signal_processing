...
where
    ...
    O: Maybe<usize>
{
    fn cheb2ap(order: O, ripple: Self::Domain) -> Self;
}
...
where
    T: Float + FloatConst,
    Zpk<Complex<T>, (), Vec<Complex<T>>, T>: Cheb2AP<usize> + System<Domain = T>
{
...
mod test
{
    ...
    use crate::{plot, Cheb2AP, Bilinear, FreqS, RealFreqZ, Zpk};
    ...
    fn test()
    {
        ...
        plot::plot_curves("H(jw)", "plots/h_s_cheb2ap.png", [&w.zip(h_f.map(|h| h.norm())), &w.zip(h_f.map(|h| h.arg()))]).unwrap();
        
        let (h_f, w): ([_; N], _) = h.bilinear(fs)
            .unwrap()
            .real_freqz(());

        plot::plot_curves("H(e^jw)", "plots/h_z_cheb2ap.png", [&w.zip(h_f.map(|h| h.norm())), &w.zip(h_f.map(|h| h.arg()))]).unwrap();
    }
}