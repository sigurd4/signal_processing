...
use crate::{Bilinear, Cheb1AP, FilterGenError, FilterGenPlane, FilterGenType, SfTrans, Sos, Ss, System, Tf, ToSos, ToSs, ToTf, Zpk};
...
where
    ...
    Zpk<Complex<T>, (), Vec<Complex<T>>, T>: Cheb1AP<usize> + for<'a> SfTrans<'a, Output = Self> + System<Domain = T>,
...
where
    T: Float + FloatConst,
    Zpk<Complex<T>, Vec<Complex<T>>, Vec<Complex<T>>, T>: Cheby1<usize> + for<'a> ToTf<'a, T, Vec<T>, Vec<T>, (), ()> + System<Domain = T>
{
    fn cheby1<const F: usize>(
        order: usize,
        ripple: T,
...
where
    T: Float + FloatConst,
    Zpk<Complex<T>, Vec<Complex<T>>, Vec<Complex<T>>, T>: Cheby1<usize> + for<'a> ToSos<'a, T, [T; 3], [T; 3], Vec<Tf<T, [T; 3], [T; 3]>>, (), ()> + System<Domain = T>
{
    fn cheby1<const F: usize>(
        order: usize,
        ripple: T,
...
where
    T: Float + FloatConst,
    Zpk<Complex<T>, Vec<Complex<T>>, Vec<Complex<T>>, T>: Cheby1<usize> + for<'a> ToSs<'a, T, Array2<T>, Array2<T>, Array2<T>, Array2<T>> + System<Domain = T>
{
    fn cheby1<const F: usize>(
        order: usize,
        ripple: T,
