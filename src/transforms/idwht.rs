...
use crate::{Lists, Matrix, OwnedList, WhtOrdering};
...
impl<'a, T, L> Idwht<T, L::RowOwned, Option<L::RowOwned>, false> for L
where
    T: ComplexFloat + MulAssign<T::Real>,
    L: Lists<T>,
    L::RowOwned: OwnedList<T>,
{
    fn idwht(self, ordering: WhtOrdering) -> Self::RowsMapped<Option<L::RowOwned>>
    {
...
impl<T, L> Idwht<T, L::RowOwned, L::RowOwned, true> for L
where
    T: ComplexFloat + MulAssign<T::Real>,
...
{
    fn idwht(self, ordering: WhtOrdering) -> Self::RowsMapped<L::RowOwned>
    {
        self.map_rows_into_owned(|mut h| {
            let h_mut = h.as_mut_slice();
            h_mut.ifwht();
...
mod test
{
    ...
    fn test()
    {
        ...
        let x: [_; _] = xf.idwht(WhtOrdering::Sequency);
