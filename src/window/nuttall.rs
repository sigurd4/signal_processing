...
mod test
{
    ...
    {
        const N: usize = 1024;
        let w: [_; N/2] = Nuttall.window_gen((), WindowRange::Symmetric);
        let n = (0.0..1.0).linspace_array();

        plot::plot_curves("g(n/N)", "plots/windows/g_n_nuttall.png", [&n.zip(w)]).unwrap();
        ...
        plot::plot_curves("G(e^jw)", "plots/windows/g_f_nuttall.png", [&omega.zip(w_f.map(|w| 20.0*w.norm().log10()))]).unwrap();
    }
}